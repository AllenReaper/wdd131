...
function search() {
    const recipeQuery = input.value.toLowerCase();

    const filteredRecipes = recipes.filter(recipe => {
        return (
            recipe.name.toLowerCase().includes(recipeQuery) ||
            recipe.author.toLowerCase().includes(recipeQuery) ||
            ...
            recipe.recipeIngredient.some(ingredient =>
                ingredient.toLowerCase().includes(recipeQuery)
            )
        );
    });
